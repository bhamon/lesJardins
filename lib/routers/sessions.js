...
/**
	@private
	@desc										Restrain the request to admin session or ensure that the request session token is the same as the auth session token.
	@param {http.Request} p_request				The request to restrain.
	@param {String} p_right						The specific admin right to check.
	@throws {module:odin/http.ExceptionRest}	403 "auth.insufficientRights" if the conditions are not met.
*/
let filterBySessionToken = function(p_request, p_right) {
...
/**
	...
	@apiGroup									sessions
	@apiPermission								admin.sessions.list
	...
	@apiError (403) auth.forbidden				If the session isn't active any more.
...
*/
let list = function(p_request, p_response, p_next) {
	p_request.initSession(lib.registry.sessionStore);
	p_request.checkSessionRight('admin.sessions.list');
	p_request.validatePagination();

	return p_request.send(
		lib.registry.sessionsStore.getPaginatedList(p_request.query.page, p_request.query.size)
...
*/
let create = function(p_request, p_response, p_next) {
	try {
		p_request.initSession(lib.registry.sessionStore);
		return p_request.send({
...
/**
	...
	@apiDescription								Returns info about a session.
												The user requires the right [admin.sessions.view] to view info about a session not owned.
	...
	@apiError (409) auth.validation				If the auth session token is malformed.
	@apiError (403) auth.forbidden				If the session isn't active any more.
	@apiError (403) auth.insufficientRights		If the session has insufficient rights or the selected requested session token doesn't match the provided auth token.
	@apiError (409) validation					If at least one parameter (query, param, body) is malformed.
...
*/
let view = function(p_request, p_response, p_next) {
	p_request.initSession(lib.registry.sessionStore);
	p_request.validate({
		params:{
	...
	filterBySessionToken(p_request, 'admin.sessions.view');
...
/**
	...
	@apiDescription								Removes a session.
												The user requires the right [admin.sessions.remove] to remove a session not owned.
	@apiParam params {String} token				Session token.
...
*/
let remove = function(p_request, p_response, p_next) {
	p_request.initSession(lib.registry.sessionStore);
	p_request.validate({
		params:{
			token:lib.registry.sessionStore.getTokenValidator()
	...
	filterBySessionToken(p_request, 'admin.sessions.remove');

	lib.registry.sessionsStore.remove(p_request.params.token);
